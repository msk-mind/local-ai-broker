...
BASE_DIR="$(mktemp -d /tmp/local-ai-broker-inspect-repo-warm-repo-state.XXXXXX)"
...
QUERY_ONE="Trace the retry_job service call chain"
QUERY_TWO="Trace how helper reaches retry_job"
...
python3 - "${BASE_DIR}" "${INPUT_REPO}" "${SHARED_CACHE_DIR}" "${QUERY_ONE}" "${QUERY_TWO}" > "${BASE_DIR}/summary.json" <<'PY'
import json
import socket
import sys
import time
...
query_one = sys.argv[4]
query_two = sys.argv[5]
spool = base_dir / "spool"
socket_path = spool / "daemon.sock"

def write_request(job_name: str, query: str, output_dir: Path):
        ...
        "input_refs": [
            {
                ...
                "content_hash": "git:warm-daemon-repo-state-proof",
            }
        ]
    }
    execution_plan = {
...
write_request("job_one", query_one, first_dir)
first = wait_result(first_dir)

write_request("job_two", query_two, second_dir)
...
assert second_runtime.get("prefetch_state_source") == "repo_state_process_cache", second_runtime
assert second_runtime.get("prefetch_state_cache_hit") is True, second_runtime

assert (first_payload.get("retrieval") or {}).get("query_stage_cache_hit") is False, first_payload
assert (second_payload.get("retrieval") or {}).get("query_stage_cache_hit") is False, second_payload

first_prefetch = float(((first_runtime.get("worker_phase_timings_ms") or {}).get("prefetch_cache_context")) or 0.0)
second_prefetch = float(((second_runtime.get("worker_phase_timings_ms") or {}).get("prefetch_cache_context")) or 0.0)
assert second_prefetch < first_prefetch, (first_prefetch, second_prefetch, second_runtime)
PY
