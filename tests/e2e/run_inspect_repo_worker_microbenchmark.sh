...
BASE_DIR="$(mktemp -d /tmp/local-ai-broker-worker-microbench.XXXXXX)"
...
def repo_case_cold():
    ...
    for i in range(samples):
        ...
        cache_dir = base_dir / f"cold-cache-{i}"
        ...
        records.append(
            {
                ...
                "build_syntax_chunks_ms": float(result["chunk_substage"].get("chunk_build_ms") or 0.0)
                + float(result["chunk_substage"].get("manifest_records_ms") or 0.0)
                + float(result["chunk_substage"].get("semantic_signature_ms") or 0.0)
                + float(result["chunk_substage"].get("symbol_marker_ms") or 0.0)
                + float(result["chunk_substage"].get("symbol_marker_write_ms") or 0.0)
                + float(result["chunk_substage"].get("source_read_ms") or 0.0)
                + float(result["chunk_substage"].get("chunk_content_hash_ms") or 0.0)
                + float(result["chunk_substage"].get("chunk_identity_hash_ms") or 0.0)
                + float(result["chunk_substage"].get("chunk_token_estimate_ms") or 0.0),
...
def repo_case_warm():
    repo = base_dir / "warm-repo"
    cache_dir = base_dir / "warm-cache"
        ...
        records.append(
            {
                ...
                "lexical_total_ms": result["lexical_total_ms"],
                "discover_source_files_ms": float(result["chunk_substage"].get("discover_source_files_ms") or 0.0),
                "snapshot_probe_ms": float(result["chunk_substage"].get("snapshot_probe_ms") or 0.0),
                "ensure_lexical_index_ms": result["lexical_total_ms"],
                "snapshot_cache_hit": bool(result["chunk_stats"].get("snapshot_cache_hit")),
            }
        )
    return records


def repo_case_partial_dirty():
    repo = base_dir / "partial-repo"
    cache_dir = base_dir / "partial-cache"
    stage_repo(repo, variant=200)
    git_init_repo(repo)
    run_build(repo, cache_dir)
    records = []
    for i in range(samples):
...
cold_records = repo_case_cold()
warm_records = repo_case_warm()
partial_dirty_records = repo_case_partial_dirty()

summary = {
    "samples": samples,
    "worker_microbenchmark": {
        "cold": {key: summarize([record[key] for record in cold_records]) for key in cold_records[0]},
        "warm": {
            **{key: summarize([record[key] for record in warm_records if not isinstance(record[key], bool)]) for key in warm_records[0] if not isinstance(warm_records[0][key], bool)},
            "snapshot_cache_hit_count": sum(1 for record in warm_records if record["snapshot_cache_hit"]),
            "records": warm_records,
        },
        "partial_dirty": {
            **{key: summarize([record[key] for record in partial_dirty_records if not isinstance(record[key], bool)]) for key in partial_dirty_records[0] if not isinstance(partial_dirty_records[0][key], bool)},
            "lexical_working_cache_hit_count": sum(1 for record in partial_dirty_records if record["lexical_working_cache_hit"]),
            "records": partial_dirty_records,
        },
    },
}
summary["worker_microbenchmark"]["cold"]["records"] = cold_records
